...
func generateDecodeEventsFunc(file *File) {
	file.Func().Id("DecodeEvents").Params(
		Id("txData").Op("*").Qual(model.PkgAgRpc, "GetTransactionResult"),
		Id("targetProgramId").Qual(model.PkgSolanaGo, "PublicKey"),
		Id("getAddressTables").Func().Params(
			Id("altAddresses").Index().Qual(model.PkgSolanaGo, "PublicKey"),
		).Params(
			Id("tables").Map(Qual(model.PkgSolanaGo, "PublicKey")).Qual(model.PkgSolanaGo, "PublicKeySlice"),
			Err().Error(),
		),
	).Params(
		...
		Var().Id("tx").Op("*").Qual(model.PkgSolanaGo, "Transaction"),
		If(
		...
		Id("altAddresses").Op(":=").Make(Index().Qual(model.PkgSolanaGo, "PublicKey"), Len(Id("tx").Dot("Message").Dot("AddressTableLookups"))),
		For(
		...
		If(Len(Id("altAddresses")).Op(">").Lit(0)).Block(
			Var().Id("tables").Map(Qual(model.PkgSolanaGo, "PublicKey")).Qual(model.PkgSolanaGo, "PublicKeySlice"),
			If(
...
func generateDecodeEventsFromEmitCPIFunc(file *File) {
	file.Func().Id("decodeEventsFromEmitCPI").Params(
		Id("InnerInstructions").Index().Qual(model.PkgAgRpc, "InnerInstruction"),
		Id("accountKeys").Qual(model.PkgSolanaGo, "PublicKeySlice"),
		Id("targetProgramId").Qual(model.PkgSolanaGo, "PublicKey"),
	).Params(
		Id("eventBinaries").Index().Index().Byte(),
		Err().Error(),
...
func generateParseEventsFunc(file *File) {
	file.Func().Id("parseEvents").Params(
		...
		Id("decoder").Op(":=").Qual(model.PkgDfuseBinary, "NewDecoderWithEncoding").Call(
			Nil(),
			Qual(model.PkgDfuseBinary, "EncodingBorsh"),
		),
		...
		For(List(Id("_"), Id("eventBinary")).Op(":=").Range().Id("base64Binaries")).Block(
			...
			Id("eventDiscriminator").Op(":=").Qual(model.PkgDfuseBinary, "TypeID").Call(Id("eventBinary").Index(Lit(0), Lit(8))),
			If(
